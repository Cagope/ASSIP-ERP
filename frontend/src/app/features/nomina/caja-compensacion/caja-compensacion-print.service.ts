...
export class CajaCompensacionPrintService {

  imprimir(items: CajaCompensacionListDTO[]): void {
  ...
  private buildHTML(items: CajaCompensacionListDTO[]): string {

    const filasHTML = items.map(x => `
      <tr>
        <td>${x.idCaja ?? ''}</td>
        <td>${(x.nombreCaja ?? '').toUpperCase()}</td>
    ...
    return `
      ...
      <head>
        <meta charset="utf-8">
        <title>Caja de Compensación</title>
      ...
      <body>
        ...
        <div class="enc">
          ...
          <div class="titulo">Nómina – Caja de Compensación</div>
        </div>
