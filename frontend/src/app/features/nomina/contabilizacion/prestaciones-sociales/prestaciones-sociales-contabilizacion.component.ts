...
import {
  PrestacionesSocialesContabilizacionApi,
  PrestacionesSocialesContabilizacionResultadoDTO
} from './prestaciones-sociales-contabilizacion.api';
...
import { PrestacionesSocialesContabilizacionExporter } from './prestaciones-sociales-contabilizacion.exporter';
...
@Component({
  standalone: true,
  selector: 'app-prestaciones-sociales-contabilizacion',
  imports: [CommonModule, FormsModule],
  templateUrl: './prestaciones-sociales-contabilizacion.component.html',
  styleUrls: ['./prestaciones-sociales-contabilizacion.component.scss']
})
export class PrestacionesSocialesContabilizacionComponent {
  ...
  resultado: PrestacionesSocialesContabilizacionResultadoDTO | null = null;
  ...
  preview(): void {
    ...
    this.api.preview(this.idPeriodoNomina as number).subscribe({
      ...
      error: (err) => {
        ...
        this.error =
          err?.error?.message ||
          'Error generando preview contable de prestaciones sociales.';
  ...
  ejecutar(): void {
    ...
    const ok = confirm(
      `¿Desea contabilizar las prestaciones sociales del mes:\n\n${descripcion}?`
    );
    ...
    this.api.ejecutar(this.idPeriodoNomina).subscribe({
      ...
      next: (resp) => {
        ...
        alert('Contabilización de prestaciones sociales realizada correctamente.');

        // limpiar proceso en pantalla
        this.movimientos = [];
        this.resumenCuentas = [];
        this.error = '';

        // refrescar comprobante existente
        this.cargarComprobanteExistente();
      },
      ...
      error: (err) => {
        ...
        this.error =
          err?.error?.message ||
          'Error ejecutando la contabilización de prestaciones sociales.';
        this.ejecutando = false;
      }
    });
  }
  ...
  reversar(): void {
    ...
    const ok = confirm(
      `¿Desea REVERSAR la contabilización de prestaciones sociales del mes:\n\n${descripcion}?\n\n⚠ Esta acción eliminará el comprobante contable generado.`
    );
    ...
    this.api.reversar(this.idPeriodoNomina).subscribe({
      ...
      next: () => {
        ...
        alert('Contabilización de prestaciones sociales reversada correctamente.');
      ...
      error: (err) => {
        ...
        this.error =
          err?.error?.message ||
          'Error reversando la contabilización de prestaciones sociales.';
  ...
  exportarExcel(): void {
    ...
    if (!this.idPeriodoNomina) {
      this.error = 'Debe seleccionar un período para identificar el mes de nómina.';
      return;
    }
