...
export class EpsPrintService {

  imprimir(items: EpsListDTO[]): void {
  ...
  private buildHTML(items: EpsListDTO[]): string {

    const filasHTML = items.map(x => `
      <tr>
        <td>${x.idEps ?? ''}</td>
        <td>${(x.nombreEps ?? '').toUpperCase()}</td>
    ...
    return `
      ...
      <body>
        ...
        <div class="enc">
          ...
          <div class="titulo">Nómina – EPS</div>
        </div>

        <h2>Listado de EPS</h2>
