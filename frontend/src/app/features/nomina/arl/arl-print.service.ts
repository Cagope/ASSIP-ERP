...
export class ArlPrintService {

  imprimir(items: ArlListDTO[]): void {
  ...
  private buildHTML(items: ArlListDTO[]): string {

    const filasHTML = items.map(x => `
      <tr>
        <td>${x.idArl ?? ''}</td>
        <td>${(x.nombreArl ?? '').toUpperCase()}</td>
    ...
    return `
      ...
      <body>
        ...
        <div class="enc">
          ...
          <div class="titulo">Nómina – ARL</div>
        </div>

        <h2>Listado de ARL</h2>
