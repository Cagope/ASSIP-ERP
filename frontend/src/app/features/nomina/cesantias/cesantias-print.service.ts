...
export class CesantiasPrintService {

  imprimir(items: CesantiasDTO[]): void {
  ...
  // ✅ HTML (IGUAL A ARL)
  // =========================================================
  private buildHTML(items: CesantiasDTO[]): string {

    const filasHTML = items.map(x => `
      <tr>
        <td>${x.idCesantias ?? ''}</td>
        <td>${(x.nombreCesantias ?? '').toUpperCase()}</td>
    ...
    return `
      ...
      <body>
        ...
        <div class="enc">
          ...
          <div class="titulo">Nómina – Cesantías</div>
        </div>

        <h2>Listado de Cesantías</h2>
