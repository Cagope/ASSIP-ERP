...
export class InteresMensualTacPrintService {
  ...
  private buildHTML(resultados: any[], filtros: any): string {
    ...
    return `
      ...
      <head>
        <meta charset="utf-8">
        <title>Interés Mensual TAC</title>
    ...
    return `
      <div class="enc">
        ...
        <div>
          ...
          <div class="tit">Proceso — Interés Mensual TAC</div>
        ...
        <tbody>
          ${rows.map(r => `
            <tr>
              ...
              <td class="right">${Number(r.promedioMensual).toLocaleString('es-CO')}</td>
              ...
              <td class="right">${Number(r.tasa).toLocaleString('es-CO')}</td>
