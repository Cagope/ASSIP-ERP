...
export class InteresMensualTacListComponent implements OnInit {
  ...
  async aplicar(): Promise<void> {
    ...
    if (!confirm(
      '¿Desea aplicar los intereses TAC?\n\n' +
  ...
  construirResumenContable(): void {
    ...
    this.resumenContable.push({
      ...
      nombre: this.formaSeleccionada.nombreCuentaFormaCorto || 'CUENTA AHORRO',
      debito: 0,
      credito: totalNeto
    });
    ...
    if (totalRetencion > 0) {
      this.resumenContable.push({
        cuenta: this.formaSeleccionada.codigoCuentaRetencionFuente || '',
        nombre: this.formaSeleccionada.nombreCuentaRetencionFuente || 'RETENCIÓN FUENTE',
        debito: 0,
