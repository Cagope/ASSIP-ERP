...
@Component({
  selector: 'app-aprobacion-analisis',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './aprobacion-analisis.component.html',
  styleUrl: './aprobacion-analisis.component.scss'
})
export class AprobacionAnalisisComponent {
  ...
  cumple(item:Record<string,unknown>,campo:string):string{return this.estado(item,campo);}
  rolAnalisis(id:unknown):string{const p=(this.deudores??[]).find(x=>x['id_solicitud_deudor']===id);return p?this.rol(p):'Deudor vinculado';}
}
