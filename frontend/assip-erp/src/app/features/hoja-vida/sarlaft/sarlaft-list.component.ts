...
import { SarlaftExportButtonComponent } from './sarlaft-export-button.component';
...
@Component({
  ...
  imports: [CommonModule, RouterLink, SarlaftExportButtonComponent],
...
export class SarlaftListComponent implements OnInit {
  ...
  item = signal<SarlaftResponse | null>(null);
  ...
  irNueva(p: PersonaItem) {
    const id = this.personaId(p); if (!id) { alert('No se encontró el identificador.'); return; }
    this.router.navigate(['/hoja-vida/sarlaft', id, 'new']);
  }
  ...
  goBackToSelector(): void { this.router.navigate(['/hoja-vida/sarlaft']); }
  idSarlaft(): number | null { return this.item()?.id_sarlaft ?? null; }

  onDelete(): void {
    const idPer = this.idDatosPersonales(); const idS = this.idSarlaft();
    if (!idPer || !idS) return;
    if (!confirm('¿Eliminar el registro?')) return;
    this.api.delete(idPer, idS).subscribe({
