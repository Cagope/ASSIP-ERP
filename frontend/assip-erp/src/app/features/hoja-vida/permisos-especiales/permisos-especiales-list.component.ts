...
import { PermisosEspecialesExportButtonComponent } from './permisos-especiales-export-button.component';
...
@Component({
  ...
  imports: [CommonModule, RouterLink, PermisosEspecialesExportButtonComponent],
...
export class PermisosEspecialesListComponent implements OnInit {
  ...
  item = signal<PermisoEspecialResponse | null>(null);
  ...
  irDetalle(p: PersonaItem) {
    ...
    this.router.navigate(['/hoja-vida/permisos-especiales', id]);
  }
  irNueva(p: PersonaItem) {
    const id = this.personaId(p); if (!id) { alert('No se encontró el identificador.'); return; }
    this.router.navigate(['/hoja-vida/permisos-especiales', id, 'new']);
  }
  ...
  goBackToSelector(): void { this.router.navigate(['/hoja-vida/permisos-especiales']); }
  idPermiso(): number | null { return this.item()?.id_permiso_especial ?? null; }

  onDelete(): void {
    const idPer = this.idDatosPersonales(); const idP = this.idPermiso();
    if (!idPer || !idP) return;
    if (!confirm('¿Eliminar el registro?')) return;
    this.api.delete(idPer, idP).subscribe({
