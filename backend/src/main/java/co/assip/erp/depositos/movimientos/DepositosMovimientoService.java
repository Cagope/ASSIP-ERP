...
public class DepositosMovimientoService {
    ...
    public DepositosMovimientoResultadoDTO registrarDebito(
        ...
        if (valor.compareTo(BigDecimal.ZERO) <= 0) {
            throw new RuntimeException(
                    "El valor débito debe ser mayor a cero."
            );
        }
        ...
        var cuenta =
                repository.obtenerCuentaConLock(
        ...
        BigDecimal saldoAnterior =
                nvl(cuenta.saldoActual());

        if (saldoAnterior.compareTo(valor) < 0) {
            throw new RuntimeException(
                    "La cuenta de ahorro no tiene saldo suficiente."
            );
        }
        ...
        repository.actualizarSaldo(
                dto.getIdCuentaAhorro(),
                saldoNuevo,
                idUsuario
        );

        repository.insertarExtracto(
                dto,
                idUsuario
        );
    ...
    public DepositosMovimientoResultadoDTO registrarCredito(
        ...
        BigDecimal valor =
                nvl(dto.getValorCredito());

        if (valor.compareTo(BigDecimal.ZERO) <= 0) {
            throw new RuntimeException(
                    "El valor crédito debe ser mayor a cero."
            );
        }
    ...
    private void registrarMovimientosMasivos(
        ...
        if (movimientos == null
            ...
            throw new RuntimeException(
                    "No se recibieron movimientos de depósitos."
            );
        }

        movimientos.forEach(this::validarBase);
        movimientos.forEach(
                this::normalizarMovimientoMasivo
        );

        List<Integer> idsCuentas =
                movimientos.stream()
                        .map(
                                DepositosMovimientoDTO::
                                        getIdCuentaAhorro
                        )
                        .distinct()
                        .toList();
        ...
        var cuentas =
                repository.obtenerCuentasConLock(
                        idsCuentas
                );
        ...
        if (cuentas.size()
            ...
            throw new RuntimeException(
                    "Una o varias cuentas de ahorro no existen."
            );
        }

        Map<Integer, BigDecimal> creditosPorCuenta =
                movimientos.stream()
                        .collect(
        ...
        Map<Integer, BigDecimal> debitosPorCuenta =
                movimientos.stream()
                        .collect(
                                Collectors.groupingBy(
                                        DepositosMovimientoDTO::
        ...
        List<DepositosMovimientoRepository.CuentaSaldoDTO>
                saldosNuevos =
                cuentas.stream()
                        .map(c -> {

                            if (validarEstado) {
                                validarCuentaOperativa(c);
                            }
                            ...
                            return new DepositosMovimientoRepository
                                    .CuentaSaldoDTO(
                                    c.idCuentaAhorro(),
                                    saldoNuevo,
                                    c.estadoCuenta(),
        ...
        repository.actualizarSaldoBatch(
                saldosNuevos,
                idUsuario
        );

        repository.insertarExtractosBatch(
                movimientos,
                idUsuario
        );
    }
    ...
    private void validarBase(
        ...
        if (dto == null) {
            throw new RuntimeException(
                    "No se recibió información del movimiento de depósitos."
            );
        }
        ...
        if (isBlank(dto.getTipoMovimiento())) {
            throw new RuntimeException(
                    "El tipo de movimiento de depósitos es obligatorio."
            );
        }

        if (isBlank(dto.getModulo())) {
            throw new RuntimeException(
                    "El módulo del movimiento de depósitos es obligatorio."
            );
        }

        dto.setTipoMovimiento(
                trim(dto.getTipoMovimiento())
        );
        ...
        if (dto.getTipoComprobante() != null) {
            dto.setTipoComprobante(
                    trim(dto.getTipoComprobante())
            );
        }

        if (dto.getNumeroComprobante() != null) {
            dto.setNumeroComprobante(
                    trim(dto.getNumeroComprobante())
            );
        }

        if (dto.getEstablecimiento() != null) {
            dto.setEstablecimiento(
                    trim(dto.getEstablecimiento())
            );
        }
    }

    private void normalizarMovimientoMasivo(
            DepositosMovimientoDTO movimiento
    ) {

        movimiento.setValorDebito(
                nvl(movimiento.getValorDebito())
        );

        movimiento.setValorCredito(
                nvl(movimiento.getValorCredito())
        );
        ...
        if (movimiento
            ...
            throw new RuntimeException(
                    "Todos los movimientos masivos deben tener valor."
            );
        }
        ...
        if (movimiento
            ...
            throw new RuntimeException(
                    "Un movimiento no puede tener débito y crédito al mismo tiempo."
            );
        }
    }

    private void validarCuentaOperativa(
            DepositosMovimientoRepository.CuentaSaldoDTO cuenta
    ) {
        ...
        if (!Boolean.TRUE.equals(
                cuenta.estadoOperativo()
        )) {

            throw new RuntimeException(
                    "La cuenta de ahorro no está operativa. Estado: "
                            + normalizarEstado(
                            cuenta.estadoCuenta()
                    )
            );
    ...
    private void validarUsuario(
        ...
        if (idUsuario == null) {
            throw new RuntimeException(
                    "El usuario del movimiento es obligatorio."
            );
        }
    }
