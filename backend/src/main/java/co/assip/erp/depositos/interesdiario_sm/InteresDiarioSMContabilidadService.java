...
public class InteresDiarioSMContabilidadService {
    ...
    public void registrarComprobante(
        ...
        Map<String, Object> forma = jdbc.queryForMap("""
            SELECT
                ...
                tasa_interes_forma::numeric AS tasa_interes_forma,
        ...
        BigDecimal tasaInteres = (BigDecimal) forma.get("tasa_interes_forma");
        BigDecimal valorMinimo = (BigDecimal) forma.get("valor_minimo");

        Integer idCuentaGasto = ((Number) forma.get("cuenta_gasto")).intValue();
        Integer idCuentaForma = ((Number) forma.get("cuenta_forma_corto")).intValue();
        ...
        if (idCuentaRetencion != null) {
            cuentaRetencion = planCuentaRepository.findById(idCuentaRetencion)
                    .orElseThrow(() -> new RuntimeException(
                            "No existe cuenta retención id: " + idCuentaRetencion
                    ));
        }

        List<MovimientoContableDTO> movimientos = new ArrayList<>();

        for (InteresDiarioSMItemDTO item : lista) {
            ...
            String detalleBase =
                    "LIQ INT DIARIO SM CTA "
                            + item.getCodigoCuenta();

            if (mayorCero(item.getInteresBruto())) {

                MovimientoContableDTO debitoGasto = new MovimientoContableDTO();
                ...
                debitoGasto.setValorDebito(nvl(item.getInteresBruto()));
                debitoGasto.setValorCredito(BigDecimal.ZERO);
                debitoGasto.setValorBase(nvl(item.getInteresBruto()));
            ...
            if (mayorCero(item.getInteresNeto())) {
                ...
                MovimientoContableDTO creditoAhorro = new MovimientoContableDTO();

                creditoAhorro.setIdCatalogoCuenta(cuentaForma.getId());
                creditoAhorro.setIdDatosPersonal(item.getIdDatosPersonal());
                creditoAhorro.setFechaAuxiliar(input.getFechaProceso());
                creditoAhorro.setDetalleMovimiento(detalleBase);
                creditoAhorro.setValorDebito(BigDecimal.ZERO);
                creditoAhorro.setValorCredito(nvl(item.getInteresNeto()));
                creditoAhorro.setValorBase(nvl(item.getInteresNeto()));

                movimientos.add(creditoAhorro);
            }

            if (cuentaRetencion != null && mayorCero(item.getRetencion())) {

                MovimientoContableDTO creditoRetencion = new MovimientoContableDTO();

                creditoRetencion.setIdCatalogoCuenta(cuentaRetencion.getId());
                creditoRetencion.setIdDatosPersonal(item.getIdDatosPersonal());
                creditoRetencion.setFechaAuxiliar(input.getFechaProceso());
                creditoRetencion.setDetalleMovimiento(
                        "RET INT DIARIO SM CTA " + item.getCodigoCuenta()
                );
                creditoRetencion.setValorDebito(BigDecimal.ZERO);
                creditoRetencion.setValorCredito(nvl(item.getRetencion()));
                creditoRetencion.setValorBase(nvl(item.getRetencion()));
        ...
        origen.setProcesoOrigen("INTERES_DIARIO_SM");
