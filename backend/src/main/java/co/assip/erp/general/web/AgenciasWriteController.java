...
public class AgenciasWriteController {

    private final AgenciaService service;
    private final UsuarioRepository usuarioRepository;

    public AgenciasWriteController(AgenciaService service, UsuarioRepository usuarioRepository) {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<Integer> create(@Valid @RequestBody AgenciaCreateRequest req) {
    ...
    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity<Void> update(@PathVariable @Min(1) Integer id, @Valid @RequestBody AgenciaUpdateRequest req) {
        Integer userId = currentUserId();
    ...
    private Integer currentUserId() {
        ...
        // Usuario.id es Long; convertirlo a Integer con validación
