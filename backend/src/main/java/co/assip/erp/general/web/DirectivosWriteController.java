...
public class DirectivosWriteController {

    private final DirectivoService service;
    private final UsuarioRepository usuarioRepository;

    public DirectivosWriteController(DirectivoService service, UsuarioRepository usuarioRepository) {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity<Integer> create(@Valid @RequestBody DirectivoCreateRequest req) {
    ...
    public ResponseEntity<Void> update(@PathVariable @Min(1) Integer id,
                                       @Valid @RequestBody DirectivoUpdateRequest req) {
        Integer userId = currentUserId();
        // ✅ Orden correcto: (id, req, userId)
