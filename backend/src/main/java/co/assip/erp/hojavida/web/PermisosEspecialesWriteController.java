...
public class PermisosEspecialesWriteController {

    private final PermisoEspecialService service;
    private final UsuarioRepository usuarioRepository;

    public PermisosEspecialesWriteController(PermisoEspecialService service, UsuarioRepository usuarioRepository) {
        this.service = service;
        this.usuarioRepository = usuarioRepository;
    }

    @PostMapping
    public ResponseEntity<?> create(@PathVariable Integer idDatosPersonal, @RequestBody PermisoEspecialRequest req) {
    ...
    public ResponseEntity<Void> update(@PathVariable Integer idDatosPersonal,
                                       ...
                                       @RequestBody PermisoEspecialRequest req) {
        Integer uid = currentUserId();
        service.updateForPersona(idDatosPersonal, idPermiso, uid, req);
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/{idPermiso}")
    public ResponseEntity<Void> delete(@PathVariable Integer idDatosPersonal, @PathVariable Integer idPermiso) {
        service.deleteForPersona(idDatosPersonal, idPermiso);
        return ResponseEntity.noContent().build();
    }
