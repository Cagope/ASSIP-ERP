...
@Service
@Transactional
public class BienInmuebleSeguroService {

    private final BienInmuebleSeguroRepository repository;
    private final UsuarioSesionService usuarioSesionService;

    public BienInmuebleSeguroService(
            BienInmuebleSeguroRepository repository,
    ...
    public List<BienInmuebleSeguro> listarPorBien(Long idBien) {
        validarIdBien(idBien);
        return repository.findByIdBienOrderByFechaVencimientoSeguroDescFechaCreacionDesc(idBien);
    }

    public Optional<BienInmuebleSeguro> buscarPorId(Long id) {
        validarIdSeguro(id);
        return repository.findById(id);
    }

    public BienInmuebleSeguro crear(BienInmuebleSeguro dto) {
        normalizar(dto);
        validar(dto);
    ...
    public Optional<BienInmuebleSeguro> actualizar(
            Long id,
            BienInmuebleSeguro dto
    ) {
        validarIdSeguro(id);
        normalizar(dto);
        validar(dto);
        ...
        return repository.findById(id).map(existente -> {
            ...
            dto.setIdBienInmuebleSeguro(id);
    ...
    private void validarIdSeguro(Long id) {
        if (id == null || id <= 0) {
            throw new IllegalArgumentException("El seguro es obligatorio.");
        }
    }

    private void validar(BienInmuebleSeguro dto) {
        if (dto == null) {
            throw new IllegalArgumentException("La información del seguro es obligatoria.");
        }
    ...
    private void normalizar(BienInmuebleSeguro dto) {
        if (dto == null) return;
