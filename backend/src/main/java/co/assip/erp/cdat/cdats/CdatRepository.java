...
public class CdatRepository {
    ...
    public void crearExtracto(Long idCuentaCdat,
                              CdatSaveDTO dto,
    ...
    public void debitarCuentaAhorro(Integer idCuentaAhorro, BigDecimal valor, Integer idUsuario) {
        String sql = """
            UPDATE depositos.cuentas_ahorro
            SET saldo_actual_cuenta = saldo_actual_cuenta - ?,
                fk_seguridad_edicion = ?,
                fecha_edicion = CURRENT_TIMESTAMP
            WHERE id_cuenta_ahorro = ?
              AND saldo_actual_cuenta >= ?
        """;

        int rows = jdbc.update(sql, valor, idUsuario, idCuentaAhorro, valor);

        if (rows == 0) {
            throw new RuntimeException("La cuenta de ahorro no tiene saldo suficiente o no existe.");
        }
    }

    public void crearExtractoAhorro(Integer idCuentaAhorro,
                                    CdatSaveDTO dto,
                                    BigDecimal valor,
                                    Integer idUsuario) {
        ...
        String sql = """
            ...
            VALUES (?, ?, CURRENT_TIME, ?, ?, '773', ?, 0, '04', 'N', 'APERTURA CDAT', ?, ?)
        """;
    ...
    """;
        ...
        if (consecutivo == null) {
            throw new RuntimeException("No existe consecutivo activo para el comprobante " + tipoComprobante);
        }
