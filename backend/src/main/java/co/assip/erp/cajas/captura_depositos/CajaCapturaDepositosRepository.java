...
public class CajaCapturaDepositosRepository {
    ...
    public Long obtenerOperacionCaja(String codigoOperacion) {
        String sql = """
    ...
    public String obtenerNaturalezaOperacion(String codigoOperacion) {
        String sql = """
            SELECT naturaleza
            FROM cajas.operaciones_caja
            WHERE codigo_operacion = ?
    ...
    public CajaCapturaDepositosCuentaDTO obtenerCuenta(Long idCuentaAhorro) {
        String sql = baseCuentaSql() + """
            WHERE ca.id_cuenta_ahorro = ?
        """;

        List<CajaCapturaDepositosCuentaDTO> datos = jdbcTemplate.query(
                sql,
    ...
    public List<CajaCapturaDepositosCuentaDTO> buscarCuentas(
            Integer idAgencia,
            String documento,
            String nombres,
        ...
        String sql = baseCuentaSql() + """
        WHERE COALESCE(ea.operativo, false) = true
        ...
        ORDER BY
            CASE WHEN ca.id_agencia = ? THEN 0 ELSE 1 END,
            ag.codigo_agencia,
            hv.primer_apellido,
            hv.segundo_apellido,
            hv.nombres,
    ...
    SELECT
        ...
        ds.numero_inicial
            AS numero_inicial_libreta,

        ds.numero_final
            AS numero_final_libreta
    ...
    private CajaCapturaDepositosCuentaDTO mapCuenta(
        ...
        return CajaCapturaDepositosCuentaDTO.builder()

                .idCuentaAhorro(
                        rs.getLong("id_cuenta_ahorro")
                )
                ...
                .saldoDisponible(
                        saldoActual.subtract(valorCanje)
                )
                ...
                .descripcionEstadoCuenta(
                        rs.getString("descripcion_estado_cuenta")
                )
                ...
                .mensajeOperativo(
                        rs.getString("mensaje_operativo")
                )

                .gmfCuenta(
                        rs.getString("gmf_cuenta")
                )

                .cuentaConjuntaReal(
                        rs.getObject("cuenta_conjunta_real") != null
                                ? rs.getBoolean("cuenta_conjunta_real")
                                : null
                )

                .conjuntos(
                        rs.getString("conjuntos")
                )

                .documentoPoder(
                        rs.getString("documento_poder")
                )

                .nombrePoder(
                        rs.getString("nombre_poder")
                )

                .telefonoPoder(
                        rs.getString("telefono_poder")
                )

                .celularPoder(
                        rs.getString("celular_poder")
                )

                .fechaAperturaCuenta(
                        rs.getString("fecha_apertura_cuenta")
                )

                .tipoDocumentoSoporte(
                        rs.getString("tipo_documento_soporte")
                )

                .numeroInicialLibreta(
                        rs.getString("numero_inicial_libreta")
                )

                .numeroFinalLibreta(
                        rs.getString("numero_final_libreta")
                )

                .firma1(
                        rs.getString("firma_uno")
                )

                .firma2(
                        rs.getString("firma_dos")
                )
    ...
    public List<TipoMovimientoDTO> listarTiposMovimientoCaja() {
        String sql = """
            SELECT
                codigo_movimiento,
                descripcion,
            ...
            WHERE COALESCE(permite_inclusion_manual, false) = false
            ORDER BY codigo_movimiento
        """;

        return jdbcTemplate.query(
                sql,
                (rs, rowNum) -> TipoMovimientoDTO.builder()
